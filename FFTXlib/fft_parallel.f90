...
!
  ...
  COMPLEX(DP), DEVICE, ALLOCATABLE :: aux_d(:), aux2_d(:)
   ...
   INTERFACE tg_cft3s
      MODULE PROCEDURE tg_cft3s_cpu
#ifdef USE_CUDA
      MODULE PROCEDURE tg_cft3s_gpu
#endif
   END INTERFACE

   INTERFACE tg_cft3s_batch
      !MODULE PROCEDURE tg_cft3s_cpu
#ifdef USE_CUDA
      MODULE PROCEDURE tg_cft3s_gpu_batch
#endif
   END INTERFACE
...
!
!----------------------------------------------------------------------------
SUBROUTINE tg_cft3s_cpu( f, dfft, isgn, dtgs )
  !----------------------------------------------------------------------------
  !
...
CONTAINS
  ...
  !
END SUBROUTINE tg_cft3s_cpu
!
#ifdef USE_CUDA
!----------------------------------------------------------------------------
SUBROUTINE tg_cft3s_gpu( f_d, dfft, isgn, dtgs )
  !----------------------------------------------------------------------------
  !
  !! ... isgn = +-1 : parallel 3d fft for rho and for the potential
  !                  NOT IMPLEMENTED WITH TASK GROUPS
  ...
  IF ( isgn > 0 ) THEN
     ...
     ELSE
        ...
        IF( use_tg ) THEN
           ...
           !CALL cft_1z( yf, dtgs%tg_nsw( me_p ), n3, nx3, isgn, aux )
        ELSE
           CALL cft_1z( f_d, dfft%nsw( me_p ), n3, nx3, isgn, aux_d )
        ENDIF
        !
        planes = dfft%iplw
        !
  ...
  ELSE
     ...
     ELSE
        ...
        IF( use_tg ) THEN
           ...
           !CALL unpack_group_sticks( yf, f, dtgs )
        ELSE
           CALL cft_1z( aux_d, dfft%nsw( me_p ), n3, nx3, isgn, f_d )
        ENDIF
        !
     ENDIF
     !
...
CONTAINS
  ...
  SUBROUTINE fw_scatter( iopt )
     ...
     IF( iopt == 2 ) THEN
        ...
        ELSE
           !
           CALL fft_scatter( dfft, aux_d, aux_h, nx3, dfft%nnr, f_d, f_h, dfft%nsw, dfft%npp, iopt )
           !
        ENDIF
        !
     ELSEIF( iopt == 1 ) THEN
        !
        CALL fft_scatter( dfft, aux_d, aux_h, nx3, dfft%nnr, f_d, f_h, dfft%nsp, dfft%npp, iopt )
        !
     ENDIF
     !
     RETURN
...
SUBROUTINE tg_cft3s_gpu_batch( f_d, dfft, isgn, batchsize )
  !----------------------------------------------------------------------------
  !
  !! ... isgn = +-1 : parallel 3d fft for rho and for the potential
  !                  NOT IMPLEMENTED WITH TASK GROUPS
  ...
  !
  USE fft_scalar, ONLY : cft_1z, cft_2xy
  USE scatter_mod,   ONLY : fft_scatter, fft_scatter_batch_a, fft_scatter_batch_b
  USE fft_types,  ONLY : fft_type_descriptor
  USE task_groups,    ONLY : task_groups_descriptor
