...
#ifdef USE_CUDA
!-----------------------------------------------------------------------
subroutine stres_ewa_gpu (alat, nat, ntyp, ityp, zv, zv_d, at, bg, tau, &
     omega, g, gg, ngm, gstart, gamma_only, gcutm, sigmaewa)
  !-----------------------------------------------------------------------
  !
  ! Ewald contribution, both real- and reciprocal-space terms are present
  ...
  real(DP) :: tau (3, nat), g (3, ngm), gg (ngm), zv (ntyp), zv_d(ntyp), &
       at (3, 3), bg (3, 3), omega, alat, gcutm, sigmaewa (3, 3)
 
  attributes(device):: g,gg,zv_d
  real(DP),allocatable,device:: tau_d(:,:)
...
12 alpha = alpha - 0.1d0
  ...
  do ng = gstart, ngm
     ...
     rhostar = (0.d0, 0.d0)
     do na = 1, nat
        arg = (g (1, ng) * tau_d (1, na) + g (2, ng) * tau_d (2, na) + &
               g (3, ng) * tau_d (3, na) ) * tpi
        rhostar = rhostar + zv_d (ityp_d (na) ) * CMPLX(cos (arg), sin (arg),kind=DP)
     enddo
     rhostar = rhostar / omega
     sewald = fact * tpi * e2 * exp ( - g2a) / g2 * abs (rhostar) **2
     sdewald = sdewald-sewald
     s11 = s11 + sewald * tpiba2 * 2.d0 * g(1,ng) * g(1,ng)/g2 * (g2a + 1)
     s21 = s21 + sewald * tpiba2 * 2.d0 * g(2,ng) * g(1,ng)/g2 * (g2a + 1)
     s22 = s22 + sewald * tpiba2 * 2.d0 * g(2,ng) * g(2,ng)/g2 * (g2a + 1)
     s31 = s31 + sewald * tpiba2 * 2.d0 * g(3,ng) * g(1,ng)/g2 * (g2a + 1)
     s32 = s32 + sewald * tpiba2 * 2.d0 * g(3,ng) * g(2,ng)/g2 * (g2a + 1)
     s33 = s33 + sewald * tpiba2 * 2.d0 * g(3,ng) * g(3,ng)/g2 * (g2a + 1)
  enddo
    sigmaewa (1,1) = s11
    sigmaewa (2,1) = s21
    sigmaewa (2,2) = s22
