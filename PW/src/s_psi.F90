...
!----------------------------------------------------------------------------
SUBROUTINE MY_ROUTINE(s_psi)( lda, n, m, psi, spsi )
  !----------------------------------------------------------------------------
  !
  ...
  IF (use_bgrp_in_hpsi .AND. .NOT. exx_is_active() .AND. m > 1) THEN
     ...
     IF (m_end >= m_start) &
        CALL MY_ROUTINE(s_psi_)( lda, n, m_end-m_start+1, psi(1,m_start), spsi(1,m_start) )
     CALL mp_sum(spsi,inter_bgrp_comm)
  ELSE
     ! don't use band parallelization here
     CALL MY_ROUTINE(s_psi_)( lda, n, m, psi, spsi )
  END IF

  CALL stop_clock( 's_psi_bgrp' )
  RETURN

END SUBROUTINE MY_ROUTINE(s_psi)
!
!----------------------------------------------------------------------------
SUBROUTINE MY_ROUTINE(s_psi_)( lda, n, m, psi, spsi )
  !----------------------------------------------------------------------------
  !
  ! ... This routine applies the S matrix to m wavefunctions psi
  ! ... and puts the results in spsi.
...
#ifdef USE_GPU
       ATTRIBUTES( DEVICE ) :: ps, qqc, becp_k
       becp_k => becp%k_d
#else
       becp_k => becp%k
#endif
...
!$cuf kernel do(2) <<<*,*>>>
                ...
                DO j = 1, nh(nt)
                   DO k = 1, nh(nt)
                      qqc(k,j) = CMPLX(qq(k,j,nt),0.0_dp, KIND=dp )
                   END DO
                END DO
#ifndef USE_GPU
!omp end here
...
END SUBROUTINE MY_ROUTINE(s_psi_)
