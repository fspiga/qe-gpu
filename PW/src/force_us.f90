...
SUBROUTINE force_us_gpu( forcenl )
  ...
  USE uspp,                 ONLY : nkb, vkb, vkb_d, qq, deeq, qq_so, deeq_nc, indv_ijkb0
  USE uspp_param,           ONLY : upf, nh, newpseudo, nhm
  USE wvfct,                ONLY : nbnd, npwx, wg, et
  USE lsda_mod,             ONLY : lsda, current_spin, isk, nspin
  USE symme,                ONLY : symvector
  ...
  REAL(DP), INTENT(OUT) :: forcenl(3,nat) ! the nonlocal contribution
  !
  COMPLEX(DP), ALLOCATABLE, device :: vkb1_d(:,:)   ! contains g*|beta>
  COMPLEX(DP), ALLOCATABLE :: deff_nc(:,:,:,:)
  REAL(DP), ALLOCATABLE :: deff(:,:,:)
  TYPE(bec_type) :: dbecp                 ! contains <dbeta|psi>
  INTEGER    :: npw, ik, ipol, ig, jkb
  ...
  DO ik = 1, nks
     ...
     DO ipol = 1, 3
        ...
        END DO
        !
        CALL calbec ( npw, vkb1_d, evc_d, dbecp )
        dbecp%k = dbecp%k_d
        !
        IF ( gamma_only ) THEN
           !
