...
SUBROUTINE h_psi_gpu( lda, n, m, psi, psi_d, hpsi, hpsi_d )
  !----------------------------------------------------------------------------
  !
  ! ... This routine computes the product of the Hamiltonian
  ! ... matrix with m wavefunctions contained in psi
...
!
!----------------------------------------------------------------------------
SUBROUTINE h_psi_gpu_( lda, n, m, psi, psi_d, hpsi, hpsi_d )
  !----------------------------------------------------------------------------
  !
  ! ... This routine computes the product of the Hamiltonian
  ! ... matrix with m wavefunctions contained in psi
...
!CUDA
  ...
  ELSE  
     ! 
     IF ( real_space .and. nkb > 0  ) then 
       print *,"REAL_SPACE NOT IMPLEMENTED FOR GPU YET!!!"
       call flush(6)
       STOP
...
!$cuf kernel do(2) <<<*,*>>>
  DO j = 1, m
    DO i = 1, n 
        hpsi_d (i, j) = hpsi_d (i, j) +  g2kin_d (i) * psi_d (i, j)
    END DO
  END DO
...
!$cuf kernel do(2) <<<*,*>>>
    DO j = 1, m
      DO i = 1, n
          hpsi_d (lda+i, j) = hpsi_d (lda+i, j) +  g2kin_d (i) * psi_d (lda+i, j)
      END DO
    END DO
  ENDIF
  !
  ...
  !
  IF ( exx_is_active() ) THEN
    print *,"NEED TO IMPLEMENT VEXX FOR GPU!!!"
    call flush(6)
    STOP
     IF ( use_ace) THEN
        IF (gamma_only) THEN
  ...
  IF ( lelfield ) THEN
     !
    print *,"LELFIELD NOT IMPLEMENTED FOR GPU YET!!!"
    call flush(6)
    STOP
