...
#ifdef USE_CUDA
SUBROUTINE s_psi_gpu( lda, n, m, psi, psi_d, spsi, spsi_d )
  !----------------------------------------------------------------------------
  !
  ! ... This routine applies the S matrix to m wavefunctions psi
  ! ... and puts the results in spsi.
  ...
  ELSE
     ! don't use band parallelization here
     CALL s_psi_gpu_( lda, n, m, psi, psi_d, spsi, spsi_d )
  END IF
...
#ifdef USE_CUDA
SUBROUTINE s_psi_gpu_( lda, n, m, psi, psi_d,  spsi, spsi_d )
  !----------------------------------------------------------------------------
  !
  ! ... This routine applies the S matrix to m wavefunctions psi
  ! ... and puts the results in spsi.
  ...
  CONTAINS
     ...
     SUBROUTINE s_psi_k_gpu()
       ...
       INTEGER :: ikb, jkb, ih, jh, na, nt, ibnd, ierr, j, k
         ! counters
       COMPLEX(DP), ALLOCATABLE :: ps(:,:), qqc(:,:)
       COMPLEX(DP), DEVICE,ALLOCATABLE :: ps_d(:,:), qqc_d(:,:)
         ! ps = product vkb and psi ; qqc = complex version of qq
       !
       ALLOCATE( ps( nkb, m ), STAT=ierr )    
       ...
       !ps(:,:) = ( 0.D0, 0.D0 )
        ps_d(:,:) = ( 0.D0, 0.D0 )
!TODO move qq_d = qq to where qq is computed
       qq_d = qq
       !
       DO nt = 1, nsp
          IF ( upf(nt)%tvanp ) THEN
...
!$cuf kernel do(2) <<<*,*>>>
                DO j = 1, nh(nt)
                   DO k = 1, nh(nt)
                      qqc_d(k,j) = CMPLX(qq_d(k,j,nt),0.0_dp, KIND=dp )
                   END DO
                END DO
...
!                       (0.0_dp,0.0_dp), ps(indv_ijkb0(na)+1,1), nkb )
                   CALL cublasZgemm('N','N', nh(nt), m, nh(nt), (1.0_dp,0.0_dp), &
                        qqc_d, nh(nt), becp%k_d(indv_ijkb0(na)+1,1), nkb, &
...
!          CALL ZGEMV( 'N', n, nkb, ( 1.D0, 0.D0 ), vkb, &
!                      lda, ps, 1, ( 1.D0, 0.D0 ), spsi, 1 )
           CALL cublasZGEMV( 'N', n, nkb, ( 1.D0, 0.D0 ), vkb_d, &
...
!          CALL ZGEMM( 'N', 'N', n, m, nkb, ( 1.D0, 0.D0 ), vkb, &
!                      lda, ps, nkb, ( 1.D0, 0.D0 ), spsi, lda )
 CALL cublasZgemm( 'N', 'N', n, m, nkb, ( 1.D0, 0.D0 ), vkb_d, &
