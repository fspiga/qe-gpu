...
#ifdef USE_CUDA
SUBROUTINE add_vuspsi_gpu( lda, n, m, hpsi, hpsi_d )
  !----------------------------------------------------------------------------
  !
  !    This routine applies the Ultra-Soft Hamiltonian to a
  !    vector psi and puts the result in hpsi.
  ...
  CONTAINS
     ...
     SUBROUTINE add_vuspsi_k()
       ...
       IMPLICIT NONE
       COMPLEX(DP), ALLOCATABLE :: ps (:,:), deeaux (:,:)
       COMPLEX(DP), DEVICE, ALLOCATABLE :: ps_d(:,:), deeaux_d(:,:)
       INTEGER :: ierr, j,k 
       !
       IF ( nkb == 0 ) RETURN
       !
...
!$cuf kernel do(2) <<<*,*>>>
                ...
                CALL cublasZgemm('N','N', nh(nt), m, nh(nt), (1.0_dp,0.0_dp), &
                             deeaux_d, nh(nt), becp%k_d(indv_ijkb0(na)+1,1), nkb, &
