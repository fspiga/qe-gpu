...
#endif
!-----------------------------------------------------------------------
subroutine MY_ROUTINE(g_psi) (lda, n, m, npol, psi, e)
  !-----------------------------------------------------------------------
  !
...
#ifdef USE_GPU
  USE cudafor
  USE g_psi_mod, ONLY : h_diag=>h_diag_d, s_diag=>s_diag_d
#else
...
!$cuf kernel do(3) <<<*,*>>>
  ...
  return
end subroutine MY_ROUTINE(g_psi)
