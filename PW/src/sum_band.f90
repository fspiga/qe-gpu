...
SUBROUTINE sum_band_gpu()
  ...
  IF ( doublegrid ) THEN
     ...
     DO is = 1, nspin
        ...
        CALL interpolate_gpu( rho%of_r_d(1,is), rho%of_r_d(1,is), 1 )
  ...
  CONTAINS
     ...
     !
     !-----------------------------------------------------------------------
     SUBROUTINE sum_band_gpu_gamma()
       !-----------------------------------------------------------------------
       !
       ! ... gamma version -- NOT PORTED TO GPU YET
       !
       USE becmod,        ONLY : becp
       USE mp_bands,      ONLY : me_bgrp
       USE mp,            ONLY : mp_sum, mp_get_comm_null
...
!$omp end parallel do
     ...
     SUBROUTINE get_rho_gpu(rho_loc, nrxxs_loc, w1_loc, psic_loc)
        ...
        INTEGER :: nrxxs_loc
        REAL(DP), DEVICE :: rho_loc(nrxxs_loc)
        REAL(DP) :: w1_loc
        COMPLEX(DP), DEVICE :: psic_loc(nrxxs_loc)
...
SUBROUTINE sum_bec_gpu ( ik, current_spin, ibnd_start, ibnd_end, this_bgrp_nbnd )
  ...
  COMPLEX(dp), DEVICE, ALLOCATABLE :: auxk1_d(:,:), auxk2_d(:,:), aux_nc_d(:,:)
  REAL(dp), DEVICE, ALLOCATABLE :: auxg_d(:,:), aux_gk_d(:,:)
  COMPLEX(dp), DEVICE, pointer :: becp_k_d(:,:)

  INTEGER :: ibnd, ibnd_loc, nbnd_loc  ! counters on bands
  INTEGER :: npw, ikb, jkb, ih, jh, ijh, na, np, is, js
  INTEGER :: cuf_ih, cuf_ibnd, myind, nhnp
  ! counters on beta functions, atoms, atom types, spin
  !
  npw = ngk(ik)
  IF ( .NOT. real_space ) THEN
...
!$cuf kernel do(2) <<<*,*>>>
                 DO cuf_ih = 1, nh(np)
                   ikb = myind + cuf_ih
                   DO cuf_ibnd = ibnd_start, ibnd_end
                      auxk1_d(cuf_ibnd,cuf_ih) = becp_k_d(ikb,cuf_ibnd)
                      auxk2_d(cuf_ibnd,cuf_ih) =  wg_d(cuf_ibnd,ik)*becp_k_d(ikb,cuf_ibnd)
                   END DO
                 END DO
                 !
                 ! only the real part is computed
